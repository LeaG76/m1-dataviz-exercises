class Fichier extends SimpleMapItem {
  // Une liste de parent pour pouvoir gérer la couleur en fonction de l'ancienneté des fichiers/dossiers
  Mappable[] listeParent;
  ...
  /* Fonction qui met à jour les couleurs des blocs des fichiers/dossiers,
   ...
   => but de l'exercice 5.3 : ici, on utilise une intensité pour indiquer
   l'âge relatif des fichiers/dossiers */
  ...
  void majCouleurs() {
    ...
    if (parent != null) {
      ...
      listeParent = ((Mappable[]) parent.getItems());
      min = ((Fichier) listeParent[0]).fichier.lastModified();
      max = ((Fichier) listeParent[0]).fichier.lastModified();
      for (int i=0; i<listeParent.length; i++) {
        if (((Fichier) listeParent[i]).fichier.lastModified() > max) {
          max = ((Fichier) listeParent[i]).fichier.lastModified();
        }
        if (((Fichier) listeParent[i]).fichier.lastModified() < min) {
          min = ((Fichier) listeParent[i]).fichier.lastModified();
        }
      }
      if (min!=max && min!=fichier.lastModified() && max!=fichier.lastModified()) {
        valeur = map(fichier.lastModified(), min-1, max+1, 20, 80);
      }
    }
